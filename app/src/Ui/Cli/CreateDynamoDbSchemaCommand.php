...
final class CreateDynamoDbSchemaCommand extends Command
{
    public function __construct(
        ...
        parent::__construct('dynamodb:create-schema');

        $this->setDescription('Creates the schema used for dev and test environments');
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($isForce) {
            $this->dropTable($this->eventStoreTableName);
            $this->dropTable($this->projectionTableName);
        }

        $this->createEventStoreTable($this->eventStoreTableName);
        $output->writeln("Created {$this->eventStoreTableName} table");
